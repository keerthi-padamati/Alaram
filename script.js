...
function run_master_clock()
{
    ...
    document.getElementById("main_clock").textContent =
        now.toLocaleTimeString('en-GB', { hour12: false });

    document.getElementById("full_date").textContent =
        now.toDateString().toUpperCase();

    document.getElementById("current_zone_id").textContent =
...
function render_alarms()
{
    ...
    box.innerHTML = alarm_storage.map((a, i) =>
        `<div class="item_row">
            <span>${a.time}</span>
            <button class="del_icon" onclick="alarm_storage.splice(${i},1);render_alarms();">🗑️</button>
        </div>`
    ).join('');
}
...
document.getElementById("sw_start_trigger").onclick = () =>
{
    ...
    sw_ref = setInterval(() =>
    {
        ...
        document.getElementById("stopwatch_timer_display").textContent =
            `${m}:${sec}:${ms}`;
    }, 10);
};
...
document.getElementById("sw_lap_trigger").onclick = () =>
{
    ...
    document.getElementById("lap_container").innerHTML =
        lap_storage.map((l, i) =>
            `<div class="item_row small py-1">
                <span>LAP ${lap_storage.length - i}</span>
    ...
    g.innerHTML = list.map(z =>
    {
        ...
        return `<div class="col-6 mb-2">
            <div class="item_row flex-column align-items-start">
                <small class="text-info">${z.split('/').pop()}</small>
                <b>${t}</b>
            </div>
        </div>`;
    }).join('');
...
{
    let total =
        (parseInt(document.getElementById("timer_min_val").value) || 0) * 60 +
        (parseInt(document.getElementById("timer_sec_val").value) || 0);
